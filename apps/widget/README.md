...
<script>
  ...
  Ahaget('event', 'data_connected');     // advances session if step has this completionEvent
  Ahaget('event', 'dashboard_created');
  Ahaget('event', 'insight_viewed');
</script>
```
...
```
src/
├── index.ts       ← Public API: Ahaget('init'|'event', ...) + replay queue
├── widget.ts      ← AhagetWidget class — orchestrates all managers
...
User completes step in your app
  → your code calls Ahaget('event', 'data_connected')
