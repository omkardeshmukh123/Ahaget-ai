    ...
    <div>
      ...
      <div className="space-y-4 mb-8">
        {OPTIONS.map((opt) => (
          ...
          >
            <div className={`mt-0.5 flex-shrink-0 ${selected === opt.value ? 'text-brand-600' : 'text-slate-400'}`}>
              {opt.icon}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-sm font-semibold ${selected === opt.value ? 'text-brand-800' : 'text-slate-800'}`}>
                  {opt.title}
                </span>
                {opt.badge && (
            ...
            {selected === opt.value && (
              <div className="mt-0.5 w-5 h-5 rounded-full bg-brand-600 flex items-center justify-center flex-shrink-0">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          disabled={loading}
          className="flex-1 py-3.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-semibold text-sm transition-all shadow-sm shadow-brand-200 disabled:opacity-50"
        >
